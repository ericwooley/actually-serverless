...
export default ({ connectionString, seed = true, onPeer }: IActuallyServerlessOptions) => {
  ...
  torrent.on("wire", wire => {
    wire.use(signal_extension(onPeer as any) as any);
  });
  return (cb: (error: Error | string) => any) => {
    torrent.destroy(cb);
    client.destroy(cb);
  };
};
function signal_extension(onPeer: (peer: Peer.Instance) => any) {
  ...
  let pushMessageToPeer: (msg: string) => any = msg => console.log("nothing to see here", msg);
        ...
        const { peer: extendedPeer, messageCallback } = initPeer(
          {
            ...
            onSignal: data => sendMessage(extendedWire, { type: MessageTypes.SIGNAL, data })
          },
          { initiator: initCheck > parsedMessage.initCheck },
          data => sendMessage(extendedWire, { type: MessageTypes.USER_MESSAGE, data })
        );
...
const initPeer = (
  ...
  let messageCallback = (message: string) => console.log("got message with no listener", message);
